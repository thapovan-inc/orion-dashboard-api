...
module.exports = class Kafka {
  startConsumer (callback) {
    consumerGroup.on('message', function (message) {
      ...
      trace['status'] = status
      trace['serviceName'] = trace.serviceNames[0]
      trace['traceName'] = trace.traceName
      trace['type'] = 'liveFeed'
      trace.startTime = new Date.now(trace.startTime/1000)
      trace.endTime = new Date.now(trace.endTime/1000)
