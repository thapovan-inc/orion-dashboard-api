...
module.exports = class Kafka {
  startConsumer (callback) {
    ...
    consumerGroup.on('message', function (message) {
      ...
      if(trace.traceEventSummary.ERROR != 0 || trace.traceEventSummary.CRITICAL != 0 || (trace.endTime === 0) || (trace.startTime === 0)) {
        status = 'FAIL'
      } else {
        if(duration > 4000) {
          status = 'SLOW'
      ...
      if(trace.startTime>0) {
        trace.startTime = trace.startTime/1000;
      } else {
        trace.startTime = "Unknown";
      }

      if(trace.endTime>0) {
        trace.endTime = trace.endTime/1000;
      } else {
        trace.endTime = "Unknown";
      }

      trace['duration'] = (duration <= 0 || duration > 86400000) ? 0 : duration
      trace['requestTime'] = (trace.startTime === "Unknown")? "Unknown" : new Date(trace.startTime).toUTCString()
      trace['status'] = status
      trace['serviceName'] = changeCase.titleCase(trace.traceName)
      trace['traceName'] = changeCase.titleCase(trace.traceName)
      trace['type'] = 'liveFeed'
